    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-primary">
                ...
                <li class="breadcrumb-item"><a href="/keuBayarDetail" class="text-white"><?= $breadcrumb[1]; ?></a></li>
        ...
        <?php if (!empty($pemb) && !empty(session()->getFlashdata('keterangan'))) : ?>
            <?php
            $angkatan = (session()->getFlashdata('keterangan')[2] == 99) ? 'Semua' : session()->getFlashdata('keterangan')[2];
            $tahap = (session()->getFlashdata('keterangan')[3] == 99) ? 'Semua' : session()->getFlashdata('keterangan')[3];
            $bnk = (session()->getFlashdata('keterangan')[4] == 99) ? 'Semua' : session()->getFlashdata('keterangan')[4]
            ?>
            <?= view('layout/templateAlert', ['msg' => ['success', 'ri-check-line', 'Data Riwayat ' . $title . ' Tahun Ajaran <strong>' . session()->getFlashdata('keterangan')[0] . '</strong> Fakultas <strong>' . session()->getFlashdata('keterangan')[1] . '</strong> Angkatan <strong>' . $angkatan . '</strong> Pembayaran Tahap <strong>' . $tahap . '</strong> Pada Bank <strong>' . $bnk . '</strong> Berhasil Dimuat!']]); ?>
        <?php elseif (empty($pemb) && !empty(session()->getFlashdata('keterangan'))) : ?>
            ...
            $bnk = (session()->getFlashdata('keterangan')[4] == 99) ? 'Semua' : session()->getFlashdata('keterangan')[4]
            ?>
            <?= view('layout/templateAlert', ['msg' => ['danger', 'ri-alert-line', 'Data Riwayat ' . $title . ' Tahun Ajaran <strong>' . session()->getFlashdata('keterangan')[0] . '</strong> Fakultas <strong>' . session()->getFlashdata('keterangan')[1] . '</strong> Angkatan <strong>' .  $angkatan . '</strong> Pembayaran Tahap <strong>' . $tahap . '</strong> Pada Bank <strong>' . $bnk . '</strong> Tidak Ditemukan!']]); ?>
        <?php endif; ?>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="col-sm-12 pb-4">
                            <form action="<?= base_url('keuBayarDetail/load') ?>" method="GET">
                                <div class="form-row">
                                    <div class="col">
                                    ...
                                    <div class="col">
                                        <select class="form-control" name="bank">
                                            <option value="99">Semua Bank</option>
                                            <?php foreach ($bank as $option) : ?>
                                                <option value="<?= $option->bankId ?>" <?= ($option->bankId == $filter[4]) ? 'selected' : '' ?>><?= $option->bankKode ?></option>
                                            <?php endforeach ?>
                                        </select>
                                    </div>
...
<div class="modal fade" id="cetak" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="/keuBayarDetail/cetak" method="post">
                <?= csrf_field(); ?>
                ...
                <input type="hidden" name="bank" value="<?= $filter[4] ?>">
