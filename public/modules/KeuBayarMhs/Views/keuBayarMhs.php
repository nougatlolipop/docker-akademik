    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-primary">
                ...
                <li class="breadcrumb-item"><a href="/keuBayarMhs" class="text-white"><?= $breadcrumb[1]; ?></a></li>
        ...
        <?php if (!empty($pemb) && !empty($mhs) && !empty(session()->getFlashdata('keterangan'))) : ?>
            <?= view('layout/templateAlert', ['msg' => ['success', 'ri-check-line', 'Data Riwayat ' . $title . ' <strong>' . session()->getFlashdata('keterangan') . '</strong> Berhasil Dimuat!']]); ?>
        <?php elseif (empty($pemb) && !empty($mhs) && !empty(session()->getFlashdata('keterangan'))) : ?>
            <?= view('layout/templateAlert', ['msg' => ['danger', 'ri-alert-line', 'Data Riwayat ' . $title . ' <strong>' . session()->getFlashdata('keterangan') . '</strong> Tidak Ditemukan!']]); ?>
        <?php endif; ?>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="/keuBayarMhs/cari" method="GET" class="searchbox">
                            <div class="row">
                                <div class="col-md-3">
                                                ...
                                                <table class="table table-bordered table-responsive-md table-striped ">
                                                    ...
                                                    <tbody>
                                                        <?php if (isset($_GET['npm']) && $_GET['npm'] != null && !empty($mhs)) : ?>
                                                            <?php $total = 0;
                                                            $no = 1;
                                                            foreach ($pemb as $row) : $total = $total + $row->nominal ?>
                                                                <tr>
                                                                    <td style="text-align:center"><?= $no++; ?></td>
                                                                    <td><?= $row->tahunAjarNama; ?></td>
                                                                    <td><?= $row->refJenisBiayaNama; ?></td>
                                                                    <td><?= $row->tahap; ?></td>
                                                                    <td><?= $row->bankKode; ?></td>
                                                                    <td><?= number_to_currency($row->nominal, 'Rp. ', 'id_ID', 2); ?></td>
                                                                </tr>
                                                            <?php endforeach ?>
                                                            <tr>
