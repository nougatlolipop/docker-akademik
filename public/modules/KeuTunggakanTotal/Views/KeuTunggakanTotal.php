    ...
    <div class="container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-primary">
                ...
                <li class="breadcrumb-item"><a href="/keuTunggakanTotal" class="text-white"><?= $breadcrumb[1]; ?></a></li>
        ...
        <?php if (!empty($tagihan) && !empty(session()->getFlashdata('keterangan'))) : ?>
            ...
            <?= view('layout/templateAlert', ['msg' => ['success', 'ri-check-line', 'Data ' . $title . ' Tahun Ajaran <strong>' . session()->getFlashdata('keterangan')[0] . '</strong> Tunggakan Tahap <strong>' . $tahap . '</strong> Berhasil Dimuat!']]); ?>
        <?php elseif (empty($tagihan) && !empty(session()->getFlashdata('keterangan'))) : ?>
            <?php $tahap = (session()->getFlashdata('keterangan')[1] == 99) ? 'Semua' : session()->getFlashdata('keterangan')[1] ?>
            <?= view('layout/templateAlert', ['msg' => ['danger', 'ri-alert-line', 'Data ' . $title . ' Tahun Ajaran <strong>' . session()->getFlashdata('keterangan')[0] . '</strong> Tunggakan Tahap <strong>' . $tahap . '</strong> Tidak Ditemukan!']]); ?>
        <?php endif; ?>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="col-sm-12 pb-4">
                            <form action="<?= base_url('keuTunggakanTotal/load') ?>" method="GET">
                                <div class="form-row">
                                    <div class="col-md-2">
                                                ...
                                                <table class="table table-bordered table-responsive-md table-striped ">
                                                    ...
                                                    <tbody>
                                                            ...
                                                            foreach ($prodi as $prd) : ?>
                                                                <?php if ($fak->fakultasId == $prd->prodiFakultasId) : ?>
                                                                    <tr>
                                                                        ...
                                                                        <?php foreach ($angkatan as $ang) : ?>
                                                                            <?php $nilai = 0;
                                                                            foreach ($tagihan as $tung) : ?>
                                                                                <?php ($ang == $tung->angkatan && $prd->prodiNama == $tung->prodiNama) ? $nilai = $tung->nominal : $nilai = $nilai ?>
                                                                                <?php if ($ang == $tung->angkatan && $prd->prodiNama == $tung->prodiNama) : ?>
                                                                                    <?php $a[$ang] = $a[$ang] + $tung->nominal; ?>
                                                                                <?php endif ?>
                                                                            <?php endforeach ?>
                                                        ...
                                                        <tr>
                                                            ...
                                                            foreach ($angkatan as $ang) : ?>
                                                                <?php $totalTunggakan = $totalTunggakan + $a[$ang] ?>
                                                            <?php endforeach ?>
                                                            <td colspan="<?= count($angkatan); ?>" style="text-align:center"><strong><?= number_to_currency($totalTunggakan, 'Rp.', 'en_ID', 0); ?></strong></td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
...
<div class="modal fade" id="cetak" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="/keuTunggakanTotal/cetak" method="post">
                <?= csrf_field(); ?>
